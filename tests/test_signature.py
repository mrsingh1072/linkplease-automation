...
def test_valid_signature_accepted():
    ...
    sig = _make_sig(body)
    assert _verify_signature(body, sig, TEST_KEY)[0] is True


def test_wrong_key_rejected():
    body = b'{"event_id":"evt1","event_type":"comment.created"}'
    sig = _make_sig(body, key="wrong_key")
    assert _verify_signature(body, sig, TEST_KEY)[0] is False
...
def test_tampered_body_rejected():
    ...
    assert _verify_signature(tampered, sig, TEST_KEY)[0] is False
...
def test_missing_sha256_prefix_rejected():
    ...
    # No 'sha256=' prefix
    assert _verify_signature(body, digest, TEST_KEY)[0] is False


def test_empty_header_rejected():
    body = b'test body'
    assert _verify_signature(body, "", TEST_KEY)[0] is False


def test_malformed_header_rejected():
    body = b'test body'
    assert _verify_signature(body, "sha256=", TEST_KEY)[0] is False
    assert _verify_signature(body, "sha256=xyz", TEST_KEY)[0] is False


def test_wrong_prefix_word_rejected():
    body = b'test body'
    digest = hmac.new(TEST_KEY.encode(), body, hashlib.sha256).hexdigest()
    assert _verify_signature(body, f"md5={digest}", TEST_KEY)[0] is False
...
def test_constant_time_comparison_used():
    ...
    assert _verify_signature(body, f"sha256={tampered_hex}", key)[0] is False
...
def test_unicode_key_handled():
    ...
    assert _verify_signature(body, sig, key)[0] is True
